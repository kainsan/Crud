<template>
  <div class="col-12 p-0 flex border-top-1 surface-border">
    <Button
      icon="pi pi-plus" severity="success" class="mr-2"
      label="Add"
  ...
  <!--  -->
    ...
    >
      <h4 class="m-0" style="color: black">Manage Products</h4>
    ...
    ></Column>
    <Column frozen alignFrozen="right" :exportable="false" style="min-width: 5rem">
      <template  #body="one">
        <Button
          icon="pi pi-pencil"
          outlined
    ...
    <template #expansion="one">
        <div class="grid">
                    <div class="col-12 lg:col-4">
                        <img :src="one.data.image" class="mb-3 w-full" />
                        
                    </div>
                    <div class="col-12 lg:col-6 py-3 lg:pl-6">
                        <div class="flex align-items-center text-xl font-medium text-900 mb-4">
                            <span>{{ one.data.name }}</span> <i class="pi ml-3 cursor-pointer" ></i>
                        </div>
                        <span class="text-900 font-medium text-3xl block mb-5">${{ one.data.price }}</span>
                        <ol class="list-none p-0 m-0 surface-100 text-center text-900">
                            <li class="line-height-3 p-3 font-bold border-bottom-1 border-300">Category:{{ one.data.category }}</li>
                            <li class="line-height-3 p-3 border-bottom-1 border-300">Description:{{ one.data.description }}</li>
                            <li class="line-height-3 p-3 border-bottom-1 border-300">Reviews: {{ one.data.review }}</li>
...
export default defineComponent({
  ...
  computed: {
    ...
    ...mapWritableState(productStore, ["one", "visible", "checked", "selected"]),
  ...
  methods: {
    ...mapActions(productStore, ["fetchData","fetchOne", "delete", "update", "create"]),
